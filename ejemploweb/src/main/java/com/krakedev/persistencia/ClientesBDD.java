...
public class ClientesBDD {
	...
	public Cliente buscarPorPK(String cedulaBusqueda) throws KrakeDevException{
		...
		try {
			...
			if(rs.next()) {
				...
				System.out.println("NOOOOO EXISTE EL CLIENTE");
			}
		} catch (KrakeDevException e) {
			e.printStackTrace();
			throw e;
	...
	public ArrayList<Cliente> buscarPorNumeroHijos(int numeroDeHijos) throws KrakeDevException{
		ArrayList<Cliente> clientes = new ArrayList<Cliente>();
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try {
			...
			while(rs.next()) {
				System.out.println("CLIENTES HALLADOS: ");
				String cedula = rs.getString("cedula");
				String nombre = rs.getString("nombre");
				int numeroHijos = rs.getInt("numeroHijos");
				cliente = new Cliente(cedula,nombre,numeroHijos);
